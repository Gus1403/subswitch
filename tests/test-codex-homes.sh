...
shim_trace="$(HOME="$fixture_root/home" SUBSWITCH_CONFIG_HOME="$config_root" bash -x "$repo_dir/bin/codex" --version 2>&1 || :)"
printf '%s\n' "$shim_trace"
...
invalid_trace="$(HOME="$fixture_root/home" SUBSWITCH_CONFIG_HOME="$config_root" bash -x "$repo_dir/bin/codex" --version 2>&1 || :)"
[[ "$invalid_trace" != *'export CODEX_HOME='* ]]

printf '%s\n%s\n' "$fixture_root/selected-home" 'unexpected second line' > "$config_root/codex-current"
corrupt_trace="$(HOME="$fixture_root/home" SUBSWITCH_CONFIG_HOME="$config_root" bash -x "$repo_dir/bin/codex" --version 2>&1 || :)"
[[ "$corrupt_trace" != *'export CODEX_HOME='* ]]

override_trace="$(HOME="$fixture_root/home" SUBSWITCH_CONFIG_HOME="$config_root" CODEX_HOME="$fixture_root/explicit" bash -x "$repo_dir/bin/codex" --version 2>&1 || :)"
